...
> Обновляется после каждой завершённой задачи. Последнее обновление: 2026-04-29.

### Блок A — Инфраструктура БД (SQLite)
...
- [x] Поиск по OEM, артикулу, кросс-номерам, названию одновременно (зависит от бэкенда)
...
### Блок E — Кнопка «Заказать» (Phase 2)
...
**ГЛОБАЛЬНЫЙ ШАГ СЕЙЧАС (Этап 1.5):**  
Миграция In-Memory каталога на локальную базу данных SQLite.  
*Цель:* Обеспечить мгновенный запуск сервера, отказоустойчивость, надежное кеширование данных от Bazon и подготовку к сложным SQL-поискам.
